...
export PATH="$USER_HOME/.local/bin:/usr/libexec/docker/cli-plugins:/usr/lib/docker/cli-plugins:$PATH"
...
if [ -f "docker-compose.yml" ] || [ -f "docker-compose.yaml" ]; then
    ...
    $DOCKER_COMPOSE pull >> /tmp/install_log.txt 2>&1
    PULL_RESULT=$?
...
$DOCKER_COMPOSE up -d builder_base builder_yolo web_server_builder >> /tmp/install_log.txt 2>&1
BUILD_RESULT=$?
...
BUILDER_BASE_ID=$($DOCKER_COMPOSE ps -q builder_base)
BUILDER_YOLO_ID=$($DOCKER_COMPOSE ps -q builder_yolo)
WEB_SERVER_BUILDER_ID=$($DOCKER_COMPOSE ps -q web_server_builder)
