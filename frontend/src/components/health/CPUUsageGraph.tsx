...
export default function CPUUsageGraph({ cpus, t }: CPUUsageGraphProps) {
  const [dataHistory, setDataHistory] = useState<CPUDataPoint[]>([]);
  ...
  const totalUsage = useMemo(() => {
    ...
    const sum = cpus.reduce((acc, cpu) => acc + cpu.usage, 0);
    return sum / cpus.length;
  }, [cpus]);
  ...
  useEffect(() => {
    ...
    const diff = Math.abs(totalUsage - previousUsageRef.current);
    ...
    animationFrameRef.current = requestAnimationFrame(() => {
      addDataPoint(totalUsage, now);

      // Update trend indicator with smoothing
      const trendDiff = totalUsage - previousUsageRef.current;
      ...
      previousUsageRef.current = totalUsage;
    });
  ...
  }, [totalUsage, addDataPoint, dataHistory.length]);
    ...
    <div className="w-full space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className={`text-2xl font-bold ${getUsageColor(totalUsage)}`}>
        ...
        <div className="text-right">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {cpus.length} {t('health', 'cores')} {t('health', 'active')}
          </p>
        </div>
      </div>
