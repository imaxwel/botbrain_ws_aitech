...
UnitreeController::UnitreeController(const UnitreeConfig& cfg)
    : cfg_(cfg),
      ...
      mode_pr_(Mode::PR),
      mode_machine_(0) {
    for (auto& packet_baseline : baseline_) {
        packet_baseline.fill(std::numeric_limits<float>::quiet_NaN());
    }
    for (auto& packet_valid : valid_) {
        packet_valid.fill(false);
    }
    ...
    // NOTE: disabled by user request to avoid forcing G1 into development mode.
    ...
    if (cfg_.hand_type == "Dex-3" && !cfg_.handstate_right_topic.empty()) {
        handstate_right_subscriber_.reset(new ChannelSubscriber<HandState_>(cfg_.handstate_right_topic));
        handstate_right_subscriber_->InitChannel(std::bind(&UnitreeController::HandStateRightHandler, this, std::placeholders::_1), 1);
        std::cout << "Subscribed right hand tactile topic: " << cfg_.handstate_right_topic << std::endl;
    }
...
void UnitreeController::HandStateRightHandler(const void* message) {
    constexpr float kInvalidValue = 30000.0f;
    constexpr float kScale = 10000.0f;

    HandState_ hand_state = *(const HandState_*)message;
    const auto& press_states = hand_state.press_sensor_state();

    Dex3RightTactileRaw raw_tactile;
    Dex3RightTactileAgg agg_tactile;

    float thumb_sum = 0.0f;
    ...
    const size_t packet_count = std::min<size_t>(press_states.size(), 9);
    for (size_t packet_idx = 0; packet_idx < packet_count; ++packet_idx) {
        const auto& press = press_states[packet_idx];
        raw_tactile.packets[packet_idx].lost = static_cast<uint64_t>(press.lost());
        raw_tactile.packets[packet_idx].reserve = static_cast<uint64_t>(press.reserve());

        if (need_calib_) {
            for (size_t taxel_idx = 0; taxel_idx < 12; ++taxel_idx) {
                const float raw_pressure = press.pressure()[taxel_idx];
                if (raw_pressure != kInvalidValue && !valid_[packet_idx][taxel_idx]) {
                    baseline_[packet_idx][taxel_idx] = raw_pressure / kScale;
                    valid_[packet_idx][taxel_idx] = true;
                    calibrated_any = true;
                }
            }
        }
        ...
        for (size_t taxel_idx = 0; taxel_idx < 12; ++taxel_idx) {
            const float raw_pressure = press.pressure()[taxel_idx];
            const float raw_temperature = press.temperature()[taxel_idx];

            if (raw_pressure == kInvalidValue) {
                raw_tactile.packets[packet_idx].pressure[taxel_idx] = std::numeric_limits<float>::quiet_NaN();
                raw_tactile.packets[packet_idx].temperature[taxel_idx] = std::numeric_limits<float>::quiet_NaN();
                continue;
            }

            const float scaled_pressure = raw_pressure / kScale;
            const float scaled_temperature = raw_temperature / kScale;
            raw_tactile.packets[packet_idx].pressure[taxel_idx] = scaled_pressure;
            raw_tactile.packets[packet_idx].temperature[taxel_idx] = scaled_temperature;

            if (!valid_[packet_idx][taxel_idx] || std::isnan(baseline_[packet_idx][taxel_idx])) {
                continue;
            }

            const float used_value = scaled_pressure - baseline_[packet_idx][taxel_idx];
            switch (taxel_idx) {
                case 0:
                case 1:
                    thumb_sum += used_value;
                    ++thumb_count;
                    break;
                case 2:
                case 3:
                    middle_sum += used_value;
                    ++middle_count;
                    break;
                case 4:
                case 5:
                    index_sum += used_value;
                    ++index_count;
                    break;
                case 6:
                case 7:
    ...
    if (need_calib_ && calibrated_any) {
        need_calib_ = false;
    }

    agg_tactile.thumb = thumb_count > 0 ? thumb_sum / static_cast<float>(thumb_count) : 0.0f;
    agg_tactile.middle = middle_count > 0 ? middle_sum / static_cast<float>(middle_count) : 0.0f;
    agg_tactile.index = index_count > 0 ? index_sum / static_cast<float>(index_count) : 0.0f;
    agg_tactile.palm = palm_count > 0 ? palm_sum / static_cast<float>(palm_count) : 0.0f;

    const float finger_palm_sum = index_sum + middle_sum + palm_sum;
    const int finger_palm_count = index_count + middle_count + palm_count;
    agg_tactile.finger_palm = finger_palm_count > 0 ? finger_palm_sum / static_cast<float>(finger_palm_count) : 0.0f;

    dex3_right_raw_buffer_.SetData(raw_tactile);
    dex3_right_agg_buffer_.SetData(agg_tactile);
}
...
void UnitreeController::step_hands(const std::vector<double>& l_hand_pose, const std::vector<double>& r_hand_pose) {
    const bool publish_left = !l_hand_pose.empty();
    const bool publish_right = !r_hand_pose.empty();

    if (!publish_left && !publish_right) {
        throw std::runtime_error("Both l_hand_pose and r_hand_pose are empty");
    }
    if (publish_left && l_hand_pose.size() != num_dofs_hand_) {
        throw std::runtime_error("l_hand_pose size mismatch");
    }
    if (publish_right && r_hand_pose.size() != num_dofs_hand_) {
        throw std::runtime_error("r_hand_pose size mismatch");
    }
    ...
    if (publish_left) {
        ...
        for (int i = 0; i < num_dofs_hand_; ++i) {
            ...
            hand_command_left_tmp.kp.at(i) = 0.5f;
    ...
    if (publish_right) {
        ...
        for (int i = 0; i < num_dofs_hand_; ++i) {
            ...
            hand_command_right_tmp.dq_target.at(i) = 0.0f;
            hand_command_right_tmp.kp.at(i) = 0.5f;
    ...
    HandCommandWriter(); // immediately send command(s)
}
...
Dex3RightTactileRaw UnitreeController::get_dex3_right_tactile_raw() {
    const std::shared_ptr<const Dex3RightTactileRaw> tactile_raw = dex3_right_raw_buffer_.GetData();

    if (tactile_raw) {
        return *tactile_raw;
    } else {
        throw std::runtime_error("Dex3 right tactile raw data is not available");
    }
}

Dex3RightTactileAgg UnitreeController::get_dex3_right_tactile_agg() {
    const std::shared_ptr<const Dex3RightTactileAgg> tactile_agg = dex3_right_agg_buffer_.GetData();

    if (tactile_agg) {
        return *tactile_agg;
    } else {
        throw std::runtime_error("Dex3 right tactile aggregate data is not available");
    }
}

#ifdef UNITREE_CPP_DEBUG_MAIN
