...
void bind_UnitreeConfig(py::module_& m) {
    py::class_<UnitreeConfig>(m, "UnitreeConfig")
        ...
        .def_readwrite("handstate_right_topic", &UnitreeConfig::handstate_right_topic)
...
void bind_Dex3RightTactile(py::module_& m) {
    py::class_<Dex3PressPacket>(m, "Dex3PressPacket")
        .def(py::init<>())
        .def_readwrite("pressure", &Dex3PressPacket::pressure)
        .def_readwrite("temperature", &Dex3PressPacket::temperature)
        .def_readwrite("lost", &Dex3PressPacket::lost)
        .def_readwrite("reserve", &Dex3PressPacket::reserve);

    py::class_<Dex3RightTactileRaw>(m, "Dex3RightTactileRaw")
        .def(py::init<>())
        .def_readwrite("packets", &Dex3RightTactileRaw::packets);

    py::class_<Dex3RightTactileAgg>(m, "Dex3RightTactileAgg")
        .def(py::init<>())
        .def_readwrite("thumb", &Dex3RightTactileAgg::thumb)
        .def_readwrite("index", &Dex3RightTactileAgg::index)
        .def_readwrite("middle", &Dex3RightTactileAgg::middle)
        .def_readwrite("palm", &Dex3RightTactileAgg::palm)
        .def_readwrite("finger_palm", &Dex3RightTactileAgg::finger_palm);
}
...
void bind_UnitreeController(py::module_& m) {
    py::class_<UnitreeController>(m, "UnitreeController")
        .def(py::init([](py::dict cfg_dict) {
            ...
            if (cfg_dict.contains("handstate_right_topic")) {
                cfg.handstate_right_topic = cfg_dict["handstate_right_topic"].cast<std::string>();
            }
        ...
        .def("get_dex3_right_tactile_raw", &UnitreeController::get_dex3_right_tactile_raw)
        .def("get_dex3_right_tactile_agg", &UnitreeController::get_dex3_right_tactile_agg);
}
