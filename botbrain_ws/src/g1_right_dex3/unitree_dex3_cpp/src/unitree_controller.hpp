...
struct Dex3PressPacket {
    ...
    Dex3PressPacket()
        : pressure{},
          ...
          lost(0),
          reserve(0) {
        pressure.fill(std::numeric_limits<float>::quiet_NaN());
        temperature.fill(std::numeric_limits<float>::quiet_NaN());
    }
};

struct Dex3RightTactileRaw {
    std::array<Dex3PressPacket, 9> packets;
};

struct Dex3RightTactileAgg {
    float thumb = 0.0f;
    float index = 0.0f;
    float middle = 0.0f;
    float palm = 0.0f;
    float finger_palm = 0.0f;
};
...
struct UnitreeConfig {
    ...
    std::string handstate_right_topic = "/lf/dex3/right/state";
...
class UnitreeController {
   public:
    ...
    Dex3RightTactileRaw get_dex3_right_tactile_raw();
    Dex3RightTactileAgg get_dex3_right_tactile_agg();
   ...
   private:
    ...
    ChannelSubscriberPtr<HandState_> handstate_right_subscriber_;
    ThreadPtr handcmd_writer_ptr_;

    DataBuffer<Dex3RightTactileRaw> dex3_right_raw_buffer_;
    DataBuffer<Dex3RightTactileAgg> dex3_right_agg_buffer_;

    std::array<std::array<float, 12>, 9> baseline_;
    std::array<std::array<bool, 12>, 9> valid_;
    bool need_calib_ = true;

    std::shared_ptr<unitree::robot::b2::MotionSwitcherClient> msc_;

    void LowStateHandler(const void* message);
    void SportStateHandler(const void* message);
    void HandStateRightHandler(const void* message);
