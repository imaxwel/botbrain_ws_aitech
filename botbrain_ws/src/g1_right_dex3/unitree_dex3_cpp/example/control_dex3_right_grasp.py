...
# User-provided fully closed pose.
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Dex-3 right hand compliant grasp demo: close until contact, then hold without "
            "continuing to push into the object."
        )
    )
    p.add_argument(
        "net_if",
    ...
    p.add_argument(
        ...
        help=f"Hold duration in seconds after contact (default: {DEFAULT_HOLD_S})",
    )
    p.add_argument(
        "--release-time",
        type=float,
        default=DEFAULT_RELEASE_S,
        help=f"Release-to-open duration in seconds (default: {DEFAULT_RELEASE_S})",
    )
    p.add_argument(
        "--contact-threshold",
        type=float,
    ...
    p.add_argument(
        "--keep-closed",
        action="store_true",
        help="Keep closed after hold instead of releasing back to open pose",
    )
    return p.parse_args()
...
def _ramp_pose(alpha: float, open_pose: Sequence[float], close_pose: Sequence[float]) -> List[float]:
    return [open_pose[i] + (close_pose[i] - open_pose[i]) * alpha for i in range(NUM_HAND_DOFS)]
...
def main() -> int:
    ...
    print(
        ...
        f"hold={args.hold:.2f}s contact_threshold={args.contact_threshold:.4f} "
    ...
    if not contact_latched:
        print("No contact detected before full close; entering hold at close pose.")

    print("State: HOLD_COMPLIANT")
    _send_pose_for_duration(ctl, hold_pose, args.rate, args.hold)

    if not args.keep_closed:
        print("State: RELEASE")
        release_steps = max(1, int(args.release_time * args.rate))
        for step in range(1, release_steps + 1):
            alpha = step / release_steps
            cmd_pose = _ramp_pose(alpha, hold_pose, open_pose)
            ctl.step_hands([], cmd_pose)
            time.sleep(dt)
        _send_pose_for_duration(ctl, open_pose, args.rate, 0.2)
