...
void GloabalLocalization::CallbackBaselink2Odom(const nav_msgs::msg::Odometry::SharedPtr baselink2odom)
{
    ...
    // Guard against upstream NaN (FAST_LIO can publish NaN/Inf odometry when
    // its EKF fails — e.g. "No Effective Points!"). Without this, NaN
    // propagates into mat_baselink2map_ → ICP → mat_odom2map_, permanently
    // poisoning the state and flooding /tf with TF_NAN_INPUT.
    if (!mat_imulink2odom.allFinite())
    {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                             "Skipping odometry callback: non-finite pose from upstream");
        return;
    }
    ...
    if (!mat_baselink2map_.allFinite() || !mat_odom2map_.allFinite())
    {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                             "Skipping odometry callback: non-finite mat_baselink2map_/mat_odom2map_");
        return;
    }
    ...
    if (loc_initialized_)
    {
        ...
        // Guard: skip broadcast when any field is non-finite, and renormalize
        // the quaternion otherwise — tf2::toMsg on a slightly non-orthogonal
        // rotation block can still produce a denormalized quaternion which
        // tf2 then rejects ("TF_DENORMALIZED_QUATERNION").
        const auto &tr = transform.transform.translation;
        auto &rot = transform.transform.rotation;
        bool finite = std::isfinite(tr.x) && std::isfinite(tr.y) && std::isfinite(tr.z) &&
                      std::isfinite(rot.x) && std::isfinite(rot.y) &&
                      std::isfinite(rot.z) && std::isfinite(rot.w);
        if (!finite)
        {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                 "Skipping map->motion_link TF: non-finite pose");
        }
        else
        {
            double qn = std::sqrt(rot.x * rot.x + rot.y * rot.y +
                                  rot.z * rot.z + rot.w * rot.w);
            if (qn < 1e-9)
            {
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                     "Skipping map->motion_link TF: zero-norm quaternion");
            }
            else
            {
                rot.x /= qn; rot.y /= qn; rot.z /= qn; rot.w /= qn;
    ...
    while (rclcpp::ok())
    {
        ...
        else
        {
            ...
            *pcd_scan2map = *source;
            ...
            // Reject a non-finite initial registration so the first downstream
            // callback doesn't immediately see NaN in mat_odom2map_.
            if (!mat_odom2map_.allFinite())
            {
                RCLCPP_WARN(this->get_logger(),
                            "LocalizationInitialize: non-finite reg_matrix, falling back to Identity");
                mat_odom2map_ = Eigen::Matrix4d::Identity();
            }
    ...
    while (rclcpp::ok())
    {
        ...
        else
        {
            ...
            *target = *map_fine_crop;
            ...
            // Plus: reject NaN/Inf reg_matrix, and reject implausibly large
            // single-step jumps (degenerate ICP can return a finite but huge
            // translation when source/target overlap collapses).
            constexpr double kMaxStepTranslation = 50.0; // meters
            Eigen::Vector3d step_xyz = (reg_matrix.block<3, 1>(0, 3) -
                                        mat_odom2map_.block<3, 1>(0, 3));
            bool reg_finite = reg_matrix.allFinite() && std::isfinite(loc_fitness_);
            bool reg_reasonable = reg_finite && step_xyz.norm() < kMaxStepTranslation;
            if (loc_fitness_ > threshold_fitness_ && reg_reasonable)
            {
                mat_odom2map_ = reg_matrix;
            }
            else if (!reg_finite)
            {
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                     "Rejecting ICP result: non-finite reg_matrix or fitness");
            }
            else if (!reg_reasonable)
            {
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                     "Rejecting ICP result: single-step jump %.2f m exceeds %.1f m",
                                     step_xyz.norm(), kMaxStepTranslation);
            }
            lock_mat_odom2map_.unlock();
