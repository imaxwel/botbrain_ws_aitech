...
bool G1Driver::play_sequence(const std::vector<UpperBodyPose>& poses, float duration_per_pose_sec)
{
    if (poses.empty()) return false;

    const float dt = 0.02f;
    int steps = static_cast<int>(duration_per_pose_sec / dt);
    if (steps < 1) steps = 1;

    // Ensure weight is 1.0 before starting
    if (!weight_is_one_.load(std::memory_order_relaxed))
    {
        UpperBodyPose first = poses[0];
        clamp_pose(first);
        for (int i = 0; i <= steps; ++i)
        {
            float w = static_cast<float>(i) / static_cast<float>(steps);
            publish_upper_body_cmd(first, w);
            std::this_thread::sleep_for(std::chrono::duration<float>(dt));
        }
        weight_is_one_.store(true, std::memory_order_relaxed);
    }

    // Continuously interpolate through all poses without stopping
    UpperBodyPose current{};
    if (!get_upper_body_pose(current)) return false;

    for (const auto& target_in : poses)
    {
        UpperBodyPose target = target_in;
        clamp_pose(target);

        for (int i = 1; i <= steps; ++i)
        {
            float alpha = static_cast<float>(i) / static_cast<float>(steps);
            UpperBodyPose cmd = smooth_pose(current, target, alpha);
            publish_upper_body_cmd(cmd, 1.0f);
            std::this_thread::sleep_for(std::chrono::duration<float>(dt));
        }
        current = target;
    }
