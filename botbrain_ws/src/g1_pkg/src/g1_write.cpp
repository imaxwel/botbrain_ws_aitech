    ...
    std::shared_ptr<bot_custom_interfaces::srv::ArmCmd::Response> response)
{
    ...
    switch (cmd)
    {
        ...
        case ArmCommand::PLAY_SEQUENCE:
        {
            std::vector<UpperBodyPose> poses;
            for (const auto& name : request->names)
            {
                UpperBodyPose pose;
                if (!get_pose_from_file(name, pose))
                {
                    response->message = "Failed to load pose: " + name;
                    return;
                }
                poses.push_back(pose);
            }

            if (!g1_driver_->play_sequence(poses, 1.0f))
            {
                response->message = "Failed to play sequence";
                break;
            }

            response->success = true;
            response->message = "Sequence completed";
            break;
        }
