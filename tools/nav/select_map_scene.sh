...
restore_previous_runtime() {
    ...
    docker compose stop loop_closure >/dev/null 2>&1 || true
...
if [ "$restart_fast_lio" = true ]; then
    ...
    docker compose stop loop_closure || true
...
while [ "$SECONDS" -lt "$scene_log_deadline" ]; do
    if docker compose logs --no-color --tail 200 localization 2>&1 |
        ...
        echo "Blue-gray=/pcd_map. Red=/scan2map candidate and /cloud_registered_1. White=/cloud_registered_body_1; live clouds appear after map TF is verified."
