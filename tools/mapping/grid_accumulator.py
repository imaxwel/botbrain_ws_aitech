...
class GridAccumulator(Node):
    ...
    def cloud_cb(self, msg: PointCloud2):
        ...
        obs_mask = (z < self.ground_z_min) | (z > self.obstacle_z)
...
def main():
    ...
    ap.add_argument("--ground-z-min", type=float, default=-0.5,
                    help="points below this are step-downs/drops and marked OCCUPIED")
    ...
    ap.add_argument("--pre-transformed", action="store_true", default=True,
                    help="cloud already in map frame; skip TF lookup")
    ap.add_argument("--no-pre-transformed", dest="pre_transformed", action="store_false",
                    help="body-frame cloud + TF mode")
    ap.add_argument("--skip-frames", type=int, default=20,
                    help="skip first N frames during fast_lio convergence warmup")
    args = ap.parse_args()
